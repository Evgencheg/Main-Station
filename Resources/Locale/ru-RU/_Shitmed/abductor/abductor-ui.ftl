abductors-ui-beacons = Маяки
abductors-ui-teleport = Телепорт
abductors-ui-attract = Притянуть
abductors-ui-experiment = Эксперимент
abductors-ui-complete-experiment = Завершить эксперимент
abductors-ui-gizmo-transferred = Информация о цели передана
abductors-ui-armor-control = Управление бронёй
abductors-ui-combat-mode = Боевой режим
abductors-ui-stealth-mode = Режим скрытности
abductors-ui-lock-armor = Заблокировать броню
abductors-ui-unlock-armor = Разблокировать броню
abductors-ui-vest-linked = Жилет подключён
abductors-title = Абдукторы
abductors-description = Абдукторы нацелились на станцию. Избегайте похищения!
abductor-lone-ghost-role-name = Одиночный Абдуктор
abductor-lone-ghost-role-desc = Похищайте людей и вживляйте им экспериментальные органы сомнительного происхождения в одиночку.
abductor-scientist-ghost-role-name = Абдуктор-учёный
abductor-scientist-ghost-role-desc = Телепортируйте людей, которых похитил ваш напарник, на ваш корабль и вживляйте им экспериментальные органы сомнительного происхождения.
abductor-agent-ghost-role-name = Абдуктор-агент
abductor-agent-ghost-role-desc = Похищайте людей для вашего напарника, чтобы он вживил им экспериментальные органы сомнительного происхождения.
abductors-ghost-role-rules = Вы - [color=red][bold]Абдуктор[/bold][/color].\\nВаша задача - похищать людей со станции и заменять их органы различными экспериментальными устройствами, после чего возвращать их обратно. Вам нельзя уничтожать станцию или намеренно убивать людей.\\nВ ваших интересах возвращать подопытных живыми и здоровыми ради чистоты эксперимента.\\n\\nВы не помните свою прошлую жизнь и ничего не помните из того, что знали как призрак.\\nРазрешено помнить только общие игровые знания, например, как готовить, как пользоваться предметами и т.д.\\nСтрого [color=red]ЗАПРЕЩЕНО[/color] помнить имя, внешность и т.п. вашего предыдущего персонажа.
abductor-round-end-agent-name = абдуктор
objective-issuer-abductors = [color=#FD0098]Материнский корабль[/color]
objective-condition-abduct-title = Похитить { $count } человек.
objective-condition-abduct-description = (используйте гизмо на обездвиженной жертве, затем используйте гизмо на консоли абдуктора и выберите действие "притянуть"), затем замените им сердце на одну из желёз, поместите в экспериментатор и нажмите "завершить эксперимент".
abductor-role-greeting = Я профессиональный боевой учёный высокотехнологичной расы. Моя задача - похищать людей, проводить над ними эксперименты и возвращать их живыми ради чистоты эксперимента. Мне невыгодно уничтожать станцию, убивать или помогать экипажу.
roles-antag-abductor-objective = Похищайте членов экипажа станции и проводите над ними эксперименты!
abductor-victim-role-greeting = Вы увидели то, что не должны были видеть. Мир должен узнать правду.
abductor-victim-role-name = Похищенный
abductor-victim-role-desc = Вы увидели то, что не должны были видеть. Мир должен узнать правду.
objective-issuer-voices = [color=#FD0098]Голоса[/color]
abductor-ui-pad-found = площадка: [color=green]подключена[/color]
abductor-ui-pad-not-found = площадка: [color=red]не найдена[/color]
abductor-ui-target-none = цель: [color=red]НЕТ[/color]
abductor-ui-target-found = цель: [color=green]{ $target }[/color]
abductor-ui-experimentator-connected = экспериментатор: [color=green]подключён[/color]
abductor-ui-experimentator-not-found = экспериментатор: [color=red]не найден[/color]
abductor-ui-victim-none = жертва: [color=red]НЕТ[/color]
abductor-ui-victim-found = жертва: [color=green]{ $victim }[/color]
abductor-ui-armor-plug-in = [color=red][font size=16]Необходимо подключить броню абдуктора![/font][/color]
